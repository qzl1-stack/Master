    ...
    , connection_state_(ConnectionState::kDisconnected)
{
    ...
    connect(local_server_.get(), &QLocalServer::newConnection, 
            this, &LocalSocketIpcCommunication::newConnection);
}
...
bool LocalSocketIpcCommunication::initialize(const QJsonObject& config)
{
    ...
    if (server_name_.isEmpty()) {
        ...
        qWarning() << "[LocalSocketIpcCommunication] 初始化失败: 配置中缺少 'server_name'";
...
bool LocalSocketIpcCommunication::start()
{
    ...
    qDebug() << "[LocalSocketIpcCommunication] 服务器已启动，监听在:" << server_name_;
    return true;
}
...
bool LocalSocketIpcCommunication::sendMessage(const IpcMessage& message)
{
    ...
    if (logical_to_internal_id_.contains(message.receiver_id)) {
        ...
        SetLastError(QString("发送消息失败: 逻辑客户端ID '%1' 未映射到内部ID").arg(message.receiver_id));
        return false;
    }

    if (clients_.find(internal_receiver_id) == clients_.end()) {
        SetLastError(QString("发送消息失败: 客户端 '%1' 不存在或未连接").arg(message.receiver_id));
        return false;
    }

    QLocalSocket* socket = clients_[internal_receiver_id].get();
    if (!socket || socket->state() != QLocalSocket::ConnectedState) {
        SetLastError(QString("发送消息失败: 客户端 '%1' 连接状态异常").arg(message.receiver_id));
        return false;
    }
    ...
    if (bytes_written == -1 || bytes_written != block.size()) {
        SetLastError(QString("发送消息到 '%1' 失败: %2").arg(message.receiver_id).arg(socket->errorString()));
        return false;
    }
    socket->flush();
    qDebug() << "[LocalSocketIpcCommunication] 消息发送成功到:" << message.receiver_id << "类型:" << static_cast<int>(message.type);
    return true;
}

bool LocalSocketIpcCommunication::sendMessage(const QString& client_id, const IpcMessage& message)
{
    QMutexLocker locker(&clients_mutex_);
    if (clients_.find(client_id) == clients_.end()) {
        SetLastError(QString("发送消息失败: 客户端 '%1' 不存在或未连接").arg(client_id));
        return false;
    }
...
bool LocalSocketIpcCommunication::broadcastMessage(const IpcMessage& message)
{
    ...
    for (auto it = clients_.begin(); it != clients_.end(); ++it) {
        ...
        if (socket && socket->state() == QLocalSocket::ConnectedState) {
            qint64 bytes_written = socket->write(block);
            if (bytes_written == -1 || bytes_written != block.size()) {
                SetLastError(QString("广播消息到 '%1' 失败: %2").arg(it->first).arg(socket->errorString()));
                all_success = false;
            }
            socket->flush();
        }
    }
    qDebug() << "[LocalSocketIpcCommunication] 广播消息完成，类型:" << static_cast<int>(message.type);
    return all_success;
}

bool LocalSocketIpcCommunication::publishToTopic(const QString& topic, const IpcMessage& message)
{
    QMutexLocker locker(&clients_mutex_);
    if (!topic_subscriptions_.contains(topic) || topic_subscriptions_[topic].isEmpty()) {
        qWarning() << "[LocalSocketIpcCommunication] 发布到Topic '" << topic << "': 没有订阅者";
    ...
    for (const QString& client_id : topic_subscriptions_[topic]) {
        if (clients_.find(client_id) != clients_.end()) {
            ...
            if (socket && socket->state() == QLocalSocket::ConnectedState) {
                qint64 bytes_written = socket->write(block);
                if (bytes_written == -1 || bytes_written != block.size()) {
                    SetLastError(QString("发布消息到Topic '%1' 给客户端 '%2' 失败: %3").arg(topic).arg(client_id).arg(socket->errorString()));
                    all_success = false;
                }
                socket->flush();
            }
    ...
    qDebug() << "[LocalSocketIpcCommunication] 发布到Topic '" << topic << "' 完成，类型:" << static_cast<int>(message.type);
    return all_success;
}

bool LocalSocketIpcCommunication::subscribeToTopic(const QString& topic)
{
    qDebug() << "[LocalSocketIpcCommunication] 订阅Topic:" << topic << " (服务器端操作)";
...
{
    qDebug() << "[LocalSocketIpcCommunication] 取消订阅Topic:" << topic << " (服务器端操作)";
...
bool LocalSocketIpcCommunication::isClientOnline(const QString& client_id) const
{
    QMutexLocker locker(&clients_mutex_);
    auto it = clients_.find(client_id);
    return it != clients_.end() && it->second->state() == QLocalSocket::ConnectedState;
}
...
void LocalSocketIpcCommunication::newConnection()
{
    ...
    while (local_server_->hasPendingConnections()) {
        std::unique_ptr<QLocalSocket> client_socket(local_server_->nextPendingConnection());
        if (client_socket) {
            QString client_id = QUuid::createUuid().toString(QUuid::WithoutBraces); // 为每个客户端生成唯一ID
            qDebug() << "[LocalSocketIpcCommunication] 新的IPC连接:" << client_id;

            connect(client_socket.get(), &QLocalSocket::disconnected, 
                    this, &LocalSocketIpcCommunication::socketDisconnected);
            connect(client_socket.get(), &QLocalSocket::readyRead, 
                    this, &LocalSocketIpcCommunication::readyRead);
            connect(client_socket.get(), QOverload<QLocalSocket::LocalSocketError>::of(&QLocalSocket::errorOccurred),
...
void LocalSocketIpcCommunication::readyRead()
{
    ...
    while (buffer.contains('\n'))
    {
        ...
        {
            qDebug() << "[LocalSocketIpcCommunication] Processing complete message:" << message_data;
            ...
            if (message.type == MessageType::kCommand && 
                (message.topic == "subscribe_topic" || message.topic == "unsubscribe_topic"))
            {
...
void LocalSocketIpcCommunication::establishIdMapping(QLocalSocket* socket, const IpcMessage& message)
{
...
void LocalSocketIpcCommunication::handleSubscriptionMessage(const IpcMessage& message)
{
    // 这个函数只处理订阅和取消订阅消息
    if (message.type == MessageType::kCommand && message.topic == "subscribe_topic")
    {
        ...
        if (!topic_to_subscribe.isEmpty() && !client_id.isEmpty())
        {
            ...
            if (!topic_subscriptions_[topic_to_subscribe].contains(client_id))
            {
                ...
                qDebug() << "[LocalSocketIpcCommunication] 客户端 '" << client_id << "' 订阅Topic:" << topic_to_subscribe;
                emit topicSubscriptionChanged(topic_to_subscribe, true);
            }
        }
    }
    else if (message.type == MessageType::kCommand && message.topic == "unsubscribe_topic")
    {
        ...
        if (!topic_to_unsubscribe.isEmpty() && !client_id.isEmpty())
        {
            ...
            qDebug() << "[LocalSocketIpcCommunication] 客户端 '" << client_id << "' 取消订阅Topic:" << topic_to_unsubscribe;
            emit topicSubscriptionChanged(topic_to_unsubscribe, false);
        }
    }
}

void LocalSocketIpcCommunication::socketError(QLocalSocket::LocalSocketError socket_error)
{
    if (shutting_down_.load(std::memory_order_relaxed)) return;
    QLocalSocket* sender_socket = qobject_cast<QLocalSocket*>(sender());
    if (!sender_socket) return;
    ...
    QString client_id = GetClientId(sender_socket);
    QString error_message = QString("Socket错误: %1").arg(sender_socket->errorString());
    SetLastError(error_message);
    qWarning() << "[LocalSocketIpcCommunication] 客户端 '" << client_id << "' 发生错误: " << error_message;
...
void LocalSocketIpcCommunication::RemoveClient(QLocalSocket* socket)
{
    ...
    if (!client_id_to_remove.isEmpty()) {
        ...
        if (internal_to_logical_id_.contains(client_id_to_remove)) {
            ...
            qDebug() << "[LocalSocketIpcCommunication] 清理ID映射: " << logical_id << "->" << client_id_to_remove;
        }
        // --- 新增代码结束 ---
        
        // 清理所有订阅中包含此客户端ID的Topic
        for (auto it = topic_subscriptions_.begin(); it != topic_subscriptions_.end(); ++it) {
...
QString LocalSocketIpcCommunication::getClientIdBySenderId(const QString& sender_id) const {
    QMutexLocker locker(&clients_mutex_);
    ...
    qDebug() << "[LocalSocketIpcCommunication] 未找到 " << sender_id << " 对应的内部客户端ID";
    return QString(); 
}
