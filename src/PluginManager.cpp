...
void PluginManager::fetchPluginList()
{
    ...
    connect(reply, &QNetworkReply::finished, this, &PluginManager::OnPluginListReply);
...
void PluginManager::OnPluginListReply()
{
    ...
    qDebug() << "[PluginManager] 插件列表响应大小:" << response_data.size() << "字节";
...
void PluginManager::ParsePluginListJson(const QJsonObject& json_obj)
{
    ...
    qDebug() << "[PluginManager] 插件列表版本:" << version << "，最后更新:" << last_update;
    ...
    qDebug() << "[PluginManager] 插件列表解析完成，共" << available_plugins_.size() << "个插件";
}

PluginManager::PluginInfo PluginManager::ParsePluginInfo(const QJsonObject& json_obj)
{
    PluginInfo info;
...
void PluginManager::installPlugin(const QString& plugin_id)
{
    ...
    connect(current_download_reply_, &QNetworkReply::downloadProgress,
            this, &PluginManager::OnDownloadProgress);
    connect(current_download_reply_, &QNetworkReply::finished,
            this, &PluginManager::OnDownloadFinished);

    qDebug() << "[PluginManager] 开始下载插件:" << plugin_id 
             << "，URL:" << plugin_info->download_url;
}

void PluginManager::OnDownloadProgress(qint64 bytes_received, qint64 bytes_total)
{
    if (bytes_total > 0) {
...
void PluginManager::OnDownloadFinished()
{
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        emit installCompleted(plugin_id, false, "下载失败: " + reply->errorString());
        return;
    }
    ...
    // 解压插件
    QString app_dir_path = QCoreApplication::applicationDirPath();
    QString extract_path = app_dir_path + "/" + kPluginsInstallDir + "/" + plugin_id;
        ...
        for (PluginInfo& info : available_plugins_) {
            if (info.id == plugin_id) {
                ...
                if (!RegisterPluginToProcessManager(info)) {
                    qWarning() << "[PluginManager] 注册插件到进程管理器失败:" << plugin_id;
                }
...
bool PluginManager::ExtractPlugin(const QString& zip_file_path, const QString& extract_path)
{
    qDebug() << "[PluginManager] 开始解压:" << zip_file_path << "到" << extract_path;
    ...
    args << "-Command" << QString("Expand-Archive -Path '%1' -DestinationPath '%2' -Force")
                            .arg(zip_file_path).arg(extract_path);
    ...
    if (unzip_process.exitCode() != 0) {
        qWarning() << "[PluginManager] 解压失败:" << unzip_process.readAllStandardError();
        return false;
    }
...
bool PluginManager::RegisterPluginToProcessManager(const PluginInfo& plugin_info)
{
    ...
    // 构建可执行文件完整路径
    QString executable_path = plugin_info.install_path + "/" + plugin_info.executable;
    ...
    // 添加到进程管理器
    bool success = process_manager.AddProcess(
        plugin_info.name,           // 进程ID（使用插件名称）
...
void PluginManager::uninstallPlugin(const QString& plugin_id)
{
    ...
    if (plugin_dir.exists()) {
        if (!plugin_dir.removeRecursively()) {
            qWarning() << "[PluginManager] 删除插件目录失败:" << plugin_info.install_path;
        }
    }
...
void PluginManager::LoadInstalledPluginsFromConfig()
{
    ...
    qDebug() << "[PluginManager] 加载了" << installed_plugins_.size() << "个已安装插件";
}
...
QVariantMap PluginManager::PluginInfoToVariantMap(const PluginInfo& info) const
{
    QVariantMap map;
    map["id"] = info.id;
    map["name"] = info.name;
